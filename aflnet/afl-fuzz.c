...
static inline void dbg_maybe_log_branch_counts(void) {
  if (unlikely(dbg_log_interval && (dbg_total_hits % dbg_log_interval == 0))) {
...
static void dbg_init_probe_from_env(void) {
  ...
  if (s && *s) {
    ...
    if (v > 0 && v < 100000000) dbg_log_interval = (uint32_t)v;
  }
}
...
/* External target mode (-X): AFLnet controls a UE via Unix socket
   instead of connecting directly to the server over TCP/UDP. */
static u8  use_external_target = 0;
static u8 *ext_target_sock     = NULL;
...
extern int apply_first_matching_rule(uint8_t* buf, size_t len);
//PART -Z 0 END 
  ...
  *out_len = 0;
  ...
  if (q->region_count) {
    u32 reg_index = UR(q->region_count);
        	...
        	//PART ALLOC 1
    if (q->regions[reg_index].start_byte > q->regions[reg_index].end_byte) {
        fprintf(stderr, "[ERROR] Invalid region: start=%u > end=%u\n", 
                q->regions[reg_index].start_byte, q->regions[reg_index].end_byte);
        *out_len = 0;
        return NULL;  
    }
    
    if (q->regions[reg_index].end_byte - q->regions[reg_index].start_byte > MAX_FILE) {
        fprintf(stderr, "[ERROR] Region too large: %u bytes\n", 
...
/* ================================================================
 * send_via_ue() — External target mode
 *
 * Replaces send_over_network(). Instead of connecting to a server
 * via TCP/UDP, this function communicates with an OAI NR UE via
 * Unix domain sockets.
 *
 * Protocol:
 *   1. AFLnet listens on ext_target_sock (e.g., /tmp/oai_fuzz.sock)
 *   2. UE connects when it reaches the fuzz hook point
 *   3. AFLnet sends: [4 bytes: msg_len (net order)][msg_len bytes: data]
 *   4. UE injects the mutated message, sends it to gNB, collects responses
 *   5. UE connects to ext_target_sock + "_resp" to send response codes
 *   6. AFLnet reads: [4 bytes: code][4 bytes: code]... [0x00000000]
 *
 * Returns 0 on success, 1 on timeout/error.
 * ================================================================ */

static int send_via_ue(void)
{
  ...
  /* --- STEP 1: Listen for UE connection --- */
  int listen_fd = socket(AF_UNIX, SOCK_STREAM, 0);
  if (listen_fd < 0) PFATAL("send_via_ue: socket() failed");

  struct sockaddr_un addr;
  memset(&addr, 0, sizeof(addr));
  addr.sun_family = AF_UNIX;
  strncpy(addr.sun_path, ext_target_sock, sizeof(addr.sun_path) - 1);
  unlink(ext_target_sock); /* remove stale socket file */

  if (bind(listen_fd, (struct sockaddr *)&addr, sizeof(addr)) < 0)
    PFATAL("send_via_ue: bind(%s) failed", ext_target_sock);
  if (listen(listen_fd, 1) < 0)
    PFATAL("send_via_ue: listen() failed");
  ...
  struct timeval tv;
  tv.tv_sec = exec_tmout / 1000;
  tv.tv_usec = (exec_tmout % 1000) * 1000;
  setsockopt(listen_fd, SOL_SOCKET, SO_RCVTIMEO, &tv, sizeof(tv));

  int ue_fd = accept(listen_fd, NULL, NULL);
  close(listen_fd);
  unlink(ext_target_sock); /* socket file no longer needed after accept */

  if (ue_fd < 0) {
    WARNF("send_via_ue: timeout waiting for UE connection on %s", ext_target_sock);
    return 1;
  }

  SAYF("[FUZZ] UE connected! Sending %d message(s)...\n", kl_messages->size);

  /* --- STEP 2: Send mutated message(s) to UE --- */
  messages_sent = 0;
  response_bytes = (u32 *)ck_alloc(sizeof(u32));

  kliter_t(lms) *it;
  int msg_idx = 0;
  for (it = kl_begin(kl_messages); it != kl_end(kl_messages); it = kl_next(it)) {
    message_t *msg = kl_val(it);

    /* Send: [4-byte length (network order)] [message data] */
    int32_t len_net = htonl(msg->msize);
    /* Hexdump first 20 bytes of data being sent */
    {
      char hex[48] = {0};
      int n = (msg->msize < 20) ? msg->msize : 20;
      for (int k = 0; k < n; k++)
        sprintf(hex + k*2, "%02x", ((u8*)msg->mdata)[k]);
      SAYF("[FUZZ] SEND msg[%d] len=%d data[0..%d]=%s\n",
           msg_idx, msg->msize, n-1, hex);
    }
    if (write(ue_fd, &len_net, 4) != 4) break;
    if (write(ue_fd, msg->mdata, msg->msize) != msg->msize) break;
    msg_idx++;

    messages_sent++;
    response_bytes = (u32 *)ck_realloc(response_bytes, messages_sent * sizeof(u32));
    response_bytes[messages_sent - 1] = 0;
  }

  /* --- STEP 3: Read gNB response codes from the SAME persistent socket ---
   * The UE keeps this socket open and sends response codes when the session
   * completes (after RRCReconfigurationComplete or RRCReject). */
  u8 gNB_crashed = 0;
  {
    /* Set read timeout on the socket */
    setsockopt(ue_fd, SOL_SOCKET, SO_RCVTIMEO, &tv, sizeof(tv));

    response_buf = NULL;
    response_buf_size = 0;

    int32_t code_net;
    while (read(ue_fd, &code_net, 4) == 4) {
      int code = ntohl(code_net);
      if (code == 0) break; /* end marker */
      if (code == 0xFF) gNB_crashed = 1; /* UE watchdog says gNB gone */

      response_buf = (char *)ck_realloc(response_buf, response_buf_size + 4);
      memcpy(response_buf + response_buf_size, &code, 4);
      response_buf_size += 4;
    }
  }

  SAYF("[FUZZ] Received %d response codes (%d bytes) from UE\n",
       response_buf_size / 4, response_buf_size);
  ...
  /* ---- Per-round fuzz log ---- */
  {
    ...
    if (flog) {
      ...
      /* Dump message types and sizes */
      fprintf(flog, "mtypes=");
      int mi = 0;
      kliter_t(lms) *it2;
      for (it2 = kl_begin(kl_messages); it2 != kl_end(kl_messages);
           it2 = kl_next(it2)) {
        message_t *m = kl_val(it2);
        if (m->msize >= 4) {
          u32 t;
          memcpy(&t, m->mdata, 4);
          fprintf(flog, "%s%d(%db)", mi ? "," : "", t, m->msize);
        }
        mi++;
      }
      /* Dump response codes */
      fprintf(flog, " codes=");
      for (int ri = 0; ri < response_buf_size / 4; ri++) {
        u32 c;
        memcpy(&c, response_buf + ri * 4, 4);
        fprintf(flog, "%s%u", ri ? "," : "", c);
      }
      fprintf(flog, "\n");
      fclose(flog);
    }
  ...
  close(ue_fd);
  return gNB_crashed ? 2 : 0; /* 2 = crash detected */
}
...
int send_over_network()
{
  ...
  for (it = kl_begin(kl_messages); it != kl_end(kl_messages); it = kl_next(it)) {
    ...
    //PART ALLOC 2
    u32 original_len = kl_val(it)->msize;
    n = net_send(sockfd, timeout, kl_val(it)->mdata, original_len);
    messages_sent++;
...
static u8 run_target(char** argv, u32 timeout) {
  ...
  /* --- External target mode (UE proxy) ---
   * Instead of forking a child and communicating over TCP, we use
   * send_via_ue() to talk to an OAI NR UE via Unix sockets. */
  if (use_external_target) {
    int ue_rc = send_via_ue();
    if (ue_rc == 1) {
      return FAULT_TMOUT;
    }
    if (ue_rc == 2) {
      /* gNB crash detected (UE watchdog saw injections=1+ responses=0) */
      SAYF("[FUZZ] gNB CRASH DETECTED!\n");
      update_state_bitmap();
    ...
    /* Update state bitmap from gNB response codes */
    update_state_bitmap();
...
static void usage(u8* argv0) {
  ...
  SAYF("\n%s [ options ] -- /path/to/fuzzed_app [ ... ]\n\n"
       ...
       //PART -Z 3
       "  -Z file       - knowledge rules JSON to enable protocol-aware hooks (PoC)\n"
       //PART -Z 3 END
...
//PART -Z 1
  while ((opt = getopt(argc, argv, "+i:o:f:m:t:T:dnCB:S:M:x:QN:D:W:w:e:P:KEq:s:X:RFc:l:b:h:Z:")) > 0)
//PART -Z 1 END
	...
	//PART -Z 2 END
      ...
      case 'P': /* protocol to be tested */
        ...
        if (!strcmp(optarg, "NGAP")) {
          extract_requests = &extract_requests_ngap;
          extract_response_codes = &extract_response_codes_ngap;
        } else {
      ...
      case 'X': /* External target mode — UE proxy via Unix socket */
        if (use_external_target) FATAL("Multiple -X options not supported");
        use_external_target = 1;
        ext_target_sock = optarg;
        break;
  ...
  if (!use_net && !use_external_target)
    FATAL("Please specify network information of the server under test (e.g., tcp://127.0.0.1/8554) or use -X for external target mode");
  ...
  //PART -Z 4
  extern void aflnet_mark_fuzzing_started(void);
  aflnet_mark_fuzzing_started();
  //PART -Z 4 END
